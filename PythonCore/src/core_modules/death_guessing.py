...
class DeathGuessingMixin(BaseMixin):
    ...
    def guess(self, message, db_session):
        ...
        if db_session.query(models.MiscValue).filter(
                        models.MiscValue.mv_key == 'guessing-enabled').one().mv_value == 'True':
    ...
    def guesstotal(self, message, db_session):
        ...
        user = self.service.get_message_display_name(message)
        if db_session.query(models.MiscValue).filter(
                        models.MiscValue.mv_key == 'guess-total-enabled').one().mv_value == "True":
            msg_list = self.service.get_message_content(message).split(' ')
            if len(msg_list) > 1:
                guess = msg_list[1]
                if guess.isdigit() and int(guess) >= 0:
