...
def SO(message):
    ...
    user = ts.get_user(message)
    me = SOCKET_ARGS['channel']
    msg_list = ts.get_hr_message(message).split(' ')
    if len(msg_list) > 1:
        channel = msg_list[1]
        ...
        for attempt in range(5):
            try:
                ...
                _add_to_chat_queue(shout_out_str)
            except requests.exceptions.HTTPError:
                _add_to_chat_queue('Hey {}, that\'s not a real streamer!'.format(user))
                break
            except ValueError:
                continue
            else:
            ...
            _add_to_chat_queue("Sorry, there was a problem talking to the twitch api. Maybe wait a bit and retry your command?")
    else:
        _add_to_chat_queue('Sorry {}, you need to specify a caster to shout out.'.format(user))